...
export const percentOf = (result: Result) => result?.progress.percent ?? 0;
...
export class UserResultService {
    ...
    async resultTree(storageKey?: string) {
        const units = await this._unitService.dataPromise;
        return await Promise.all(units.map(async (unit, index) => {
