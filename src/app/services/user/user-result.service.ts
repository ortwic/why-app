...
import { TOTAL_BONUS_POINTS, UserDataService } from './user-data.service';
...
export class UserResultService {
    ...
    readonly results = toSignal(this.unitService.getUnits().pipe(
        map(units => units.map((unit) => this.calcUnitResult(unit)))
    ), { initialValue: [] });
    ...
    calcUnitResult(unit: UnitView, bonusPoints = TOTAL_BONUS_POINTS): UnitResults {
